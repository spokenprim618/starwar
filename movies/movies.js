...
async function fetchNestedData(url) {
    try {
        ...
        if (match) {
            const { fields } = match;
            const limitedData = fields
                .map((field) => `${field}: ${data[field] || "N/A"}`)
                .join(", ");
            return limitedData;
        }
        ...
        const content = await Promise.all(
            Object.entries(item).map(async ([key, value]) => {
                if (Array.isArray(value)) {
                    if (value.length === 0) {
                        return `<div class=${key}><h3>${key}</h3><p>Information unavailable </p></div>`;
                    }
                    ...
                    return `<div class=${key}><h3>${key}</h3><p>${value || "Information unavailable "}</p></div>`;
                }
            })
        );
